...
pub fn sha384_hash(msg: &mut Vec<u8>) -> ~[u64] {
    let mut hash = sha384_begin();
...
pub fn sha512224_hash(msg: &mut Vec<u8>) -> ~[u64] {
    let mut hash = sha512224_begin();
...
pub fn sha512256_hash(msg: &mut Vec<u8>) -> ~[u64] {
    let mut hash = sha512256_begin();
...
pub fn sha512224_dump(hash: &[u64]) {
    let last: u32 = u32::from_be(u64::to_be(hash[3]).slice(0, 4));
    println!("{:016x}{:016x}{:016x}{:08x}",
             hash[0],
             hash[1],
             hash[2],
             last);
...
pub fn sha512256_dump(hash: &[u64]) {
...
pub fn main_512_224() {
    let mut reader = stdin();
    let mut msg = reader.read_to_end().unwrap();
    let hash = sha512224_hash(&mut msg);
    sha512224_dump(hash);
}
...
pub fn main_512_256() {
    let mut reader = stdin();
    let mut msg = reader.read_to_end().unwrap();
    let hash = sha512256_hash(&mut msg);
    sha512256_dump(hash);
}
